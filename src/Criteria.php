 ...
 * - whitelist filtrů: [ 'user_id', 'profile_enc', 'key_version', 'encryption_meta', 'updated_at' ]
 * - whitelist pro LIKE hledání: [ 'key_version' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'user_id', 'profile_enc', 'key_version', 'encryption_meta', 'updated_at' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'user_id', 'profile_enc', 'key_version', 'encryption_meta', 'updated_at' ], true);
            $parts = [];
