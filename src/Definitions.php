...
final class Definitions {
    ...
    /** @return string[] */
    public static function columns(): array { return [ 'user_id', 'profile_enc', 'key_version', 'encryption_meta', 'updated_at', 'version' ]; }
     ...
     * user_id may be "id" or "col1, col2".
     * @return string[]
     */
    public static function pkColumns(): array {
        $raw = trim('user_id');
        if ($raw === '') {
            throw new \InvalidArgumentException('Definitions::pkColumns(): token user_id must not be empty.');
        }
    ...
    public static function defaultOrder(): ?string {
        $c = trim('user_id DESC'); return $c !== '' ? $c : null;
    }

    /** @return array<int,array<int,string>> list of unique keys */
    public static function uniqueKeys(): array { return [ [ 'user_id' ] ]; }
