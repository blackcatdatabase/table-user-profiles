...
use BlackCat\Database\Packages\UserProfiles\Repository\UserProfileRepositoryInterface;
...
class UserProfileRepository implements UserProfileRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'user_id' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\UserProfiles\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'profile_enc', 'key_version', 'encryption_meta' ]);
          return count($rows);
        }
