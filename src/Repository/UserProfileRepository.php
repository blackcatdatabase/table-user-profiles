...
namespace BlackCat\Database\Packages\UserProfiles\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\UserProfiles\Definitions;
use BlackCat\Database\Packages\UserProfiles\Criteria;
use BlackCat\Database\Packages\UserProfiles\Dto\UserProfileDto as Dto;
use BlackCat\Database\Packages\UserProfiles\Mapper\UserProfileDtoMapper as RowMapper;
...
final class UserProfileRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\UserProfiles\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'profile_enc', 'key_version', 'encryption_meta' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'user_id' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\UserProfiles\Definitions::table());
            return $bulk->upsertMany($rows, [ 'user_id' ], [ 'profile_enc', 'key_version', 'encryption_meta' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\UserProfiles\Dto\UserProfileDto|null */
    public function getByUserId(int $userId, bool $asDto = false): array|\BlackCat\Database\Packages\UserProfiles\Dto\UserProfileDto|null {
        $row = $this->getByUnique([ 'user_id' => $userId ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\UserProfiles\Mapper\UserProfileDtoMapper::fromRow($row);
    }
    public function existsByUserId(int $userId): bool {
        $where = 't.' . Ident::q($this->db, 'user_id') . ' = :uniq_user_id';
        return $this->exists($where, [ 'uniq_user_id' => $userId ]);
    }
    /** @return int|string|null */
    public function getIdByUserId(int $userId) {
