...
use BlackCat\Database\Packages\UserProfiles\ContractRepository;

final class UserProfileRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO user_profiles (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO user_profiles (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO user_profiles (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE user_profiles SET ".implode(',', $assign)." WHERE user_id = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE user_profiles SET $set WHERE user_id = :id", $params);
        }
        return $this->db->execute("DELETE FROM user_profiles WHERE user_id = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE user_profiles SET $set WHERE user_id = :id", [':id'=>$id]);
    }
    ...
    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM user_profiles WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM user_profiles WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM user_profiles t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM user_profiles t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM user_profiles WHERE user_id = :id FOR UPDATE"
            : "SELECT * FROM user_profiles WHERE user_id = :id FOR UPDATE";
